...
class Task(Base):
    ...
    target_site_id = Column(sa.Uuid(as_uuid=True), ForeignKey("sites.id"), nullable=False, index=True)
    author_id = Column(BigInteger, ForeignKey("authors.id"), nullable=True)
    status = Column(String(20), default="pending", nullable=False, index=True)
    total_cost = Column(Float, default=0.0)
    error_log = Column(Text, nullable=True)
    serp_data = Column(sa.JSON, nullable=True)
    competitors_text = Column(Text, nullable=True)
    outline = Column(sa.JSON, nullable=True)
    additional_keywords = Column(Text, nullable=True)
    priority = Column(Integer, default=0)
    retry_count = Column(Integer, default=0)
    step_results = Column(sa.JSON, nullable=True, default=dict)
    serp_config = Column(sa.JSON, nullable=True, default=dict)
    ...
    project_id = Column(sa.Uuid(as_uuid=True), ForeignKey("site_projects.id"), nullable=True, index=True)
    blueprint_page_id = Column(sa.Uuid(as_uuid=True), ForeignKey("blueprint_pages.id"), nullable=True)
