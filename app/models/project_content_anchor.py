...
class ProjectContentAnchor(Base):
    ...
    project_id = Column(UUID(as_uuid=True), ForeignKey('site_projects.id', ondelete='CASCADE'), nullable=True, index=True)
    task_id = Column(UUID(as_uuid=True), ForeignKey('tasks.id', ondelete='CASCADE'), nullable=True, unique=True)
