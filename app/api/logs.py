...
LOG_PATTERN = re.compile(
    r"^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}(?:,\d{3})?) - (.*?) - (INFO|WARNING|ERROR|DEBUG|CRITICAL) - (.*)$"
)
            ...
            "logs": [
                {
                    ...
                    "level": "INFO",
                    "service": "system",
                    "message": "Log file not found or not created yet.",
                }
            ]
        }
        ...
        with open(LOG_FILE_PATH) as f:
            ...
            for line in all_lines:
                ...
                if match:
                    ...
                    if level == "ALL":
                        parsed_logs.append(
                            {"timestamp": "", "service": "unknown", "level": "INFO", "message": line_str}
                        )
