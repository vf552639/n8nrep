...
def build_site(db: Session, project_id: str) -> str:
    ...
    for task in tasks:
        ...
        full_raw = (article.full_page_html or "").strip() if article else ""
        body_fallback = (article.html_content or "").strip() if article else ""
        if article and not full_raw and body_fallback:
            logger.warning(
                f"[build_site] task {task.id}: full_page_html empty, falling back to html_content (meta may be missing)"
            )
        page_html = full_raw or body_fallback
        if not article or not page_html:
            logger.warning(f"[build_site] No article HTML for task {task.id} (keyword: {task.main_keyword})")
            continue
    ...
    logger.info(f"[build_site] {len(written_files)} HTML files in temp dir, creating ZIP...")
