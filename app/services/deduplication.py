...
STOP_WORDS = set([
    ...
    "on", "at", "to", "for", "with", "about", "as", "by", "this", "that", 
    "it", "he", "she", "they", "we", "you", "i", "of", "from"
])
...
class ContentDeduplicator:
    ...
    def save_anchors(self, project_id: str, task_id: str, anchors: dict):
        anchor_obj = self.db.query(ProjectContentAnchor).filter(ProjectContentAnchor.task_id == task_id).first()
    ...
    def get_already_covered(self, project_id: str, current_task_id: str) -> str:
        ...
        anchors = self.db.query(ProjectContentAnchor).filter(
            ProjectContentAnchor.project_id == project_id,
            ProjectContentAnchor.task_id != current_task_id
        ).order_by(desc(ProjectContentAnchor.created_at)).limit(20).all()
