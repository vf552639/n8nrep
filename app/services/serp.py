...
def call_dataforseo(
    ...
    headers = {"Authorization": _get_dataforseo_auth_header(), "Content-Type": "application/json"}
    ...
    print(
        f"[DataForSEO REQUEST] engine=google, keyword='{keyword}', "
    ...
    try:
        ...
        if data and isinstance(data, dict) and data.get("tasks") and len(data["tasks"]) > 0:
            ...
            print(
                f"[DataForSEO RESPONSE] status_code={response.status_code}, "
...
def call_dataforseo_bing(
    ...
    print(
        f"[DataForSEO REQUEST] engine=bing, keyword='{keyword}', "
        f"raw_location='{location_code}' → mapped={dfs_loc}, "
        f"raw_language='{language_code}' → mapped='{dfs_lang}', "
        f"depth={depth}, device={device}, os={os_type}"
    )
    ...
    try:
        ...
        if data and isinstance(data, dict) and data.get("tasks") and len(data["tasks"]) > 0:
            ...
            items_count = len(task_result[0].get("items", [])) if task_result else 0
            print(
                f"[DataForSEO Bing RESPONSE] status_code={response.status_code}, "
                f"task_status_code={task0.get('status_code')}, "
                f"task_status_message='{task0.get('status_message')}', "
                f"result_count={len(task_result)}, items_count={items_count}"
            )
...
def call_serpapi(keyword: str, location: str, language: str) -> dict[str, Any] | None:
    url = "https://serpapi.com/search"
    params = {
        "engine": "google",
        "q": keyword,
        "gl": _map_location_serpapi(location)
        if len(location) == 2
        else location,  # gl mostly expects 2-letter country code
        "hl": _map_language_serpapi(language),
        "num": 10,
        "api_key": settings.SERPAPI_KEY,
    }
    print(
        f"[SerpAPI REQUEST] keyword='{keyword}', "
    ...
    try:
        ...
        data = response.json()
        print(
            f"[SerpAPI RESPONSE] status_code={response.status_code}, "
...
def _merge_serp_results(google_result: dict, bing_result: dict) -> dict:
    ...
    merged["serp_features"] = sorted(
        set(merged.get("serp_features", [])) | set(bing_result.get("serp_features", []))
    )
...
def _fetch_serp_data_uncached(
    keyword: str, country_code: str, language_code: str, serp_config: dict = None
) -> dict[str, Any]:
    """
    ...
    print(
        f"[SERP] fetch_serp_data called: keyword='{keyword}', "
        f"country='{country_code}', language='{language_code}', "
        f"engine='{engine}', depth={depth}, device='{device}', os='{os_type}'"
    )

    if engine == "bing":
        dfs_data = call_dataforseo_bing(keyword, country_code, language_code, depth, device, os_type)
        ...
        dfs_google = call_dataforseo(keyword, country_code, language_code, depth, device, os_type)
        if dfs_google and dfs_google.get("items"):
            google_result = _parse_dataforseo_response(dfs_google)

        dfs_bing = call_dataforseo_bing(keyword, country_code, language_code, depth, device, os_type)
    ...
    for attempt in range(MAX_SERP_RETRIES):
        dfs_parsed = None
        dfs_data = call_dataforseo(keyword, country_code, language_code, depth, device, os_type)
        if dfs_data and dfs_data.get("items"):
        ...
        if attempt < MAX_SERP_RETRIES - 1:
            print(f"[SERP] Attempt {attempt + 1} failed, retrying in {SERP_RETRY_DELAY}s...")
...
def check_serp_health() -> dict:
    ...
    if settings.DATAFORSEO_LOGIN and settings.DATAFORSEO_PASSWORD:
        start = time_mod.time()
        try:
            dfs_data = call_dataforseo("test", "us", "en", depth=1, device="mobile", os_type="android")
