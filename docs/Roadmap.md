...
Операционный срез приоритетов (перенесено из **`docs/CURRENT_STATUS.md`**, май 2026). Детальные спецификации частично пересекаются с квартальными разделами (**Q2–Q4**) ниже; при противоречии ориентир — описание в соответствующем квартале / подразделе приоритетов.

**Приоритет 1 (критично — стабильность):**
1. Quality Gate — автовалидация вывода каждого LLM-шага (min length, HTML tags, JSON validity)
2. Fallback-модель — при сбое основной модели автоматически переключаться на резервную *(частично закрыто маршрутизацией OpenRouter — см. **task52**, **`LLM_MODEL_FALLBACKS`**)*
...
Ниже — короткий список из roadmap-сессии **`CURRENT_STATUS`**; часть пунктов продублирована в **Q1 2027 (Backlog)** в конце файла.
