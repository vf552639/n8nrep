...
export default function SerpUrlsReviewer({ taskId, onApproved }: Props) {
  ...
  const fetchMetaForRow = async (url: string) => {
    setRows((prev) =>
      prev.map((row) =>
        row.url === url ? { ...row, meta_loading: true, meta_error: false } : row,
      ),
    );
    try {
      const meta = await tasksApi.fetchUrlMeta(url);
      setRows((prev) =>
        prev.map((row) =>
          row.url === url
            ? {
      ...
      setRows((prev) =>
        prev.map((row) =>
          row.url === url ? { ...row, meta_loading: false, meta_error: true } : row,
        ),
      );
    }
  };
              ...
              <Fragment key={row.url + idx}>
                <tr className="border-t border-slate-100">
                  ...
                  <td className="max-w-[220px] px-3 py-2 align-top">
                    <span className="line-clamp-2 font-medium text-blue-700">{row.title || "—"}</span>
                  </td>
                  <td className="max-w-[260px] px-3 py-2 align-top text-slate-600">
                    {row.meta_loading ? (
                  ...
                  <td className="px-2 py-2 align-top">
                    <div className="flex items-center gap-1">
                      <button
                        type="button"
                        onClick={() => void fetchMetaForRow(row.url)}
                        className="rounded p-1.5 text-slate-500 hover:bg-sky-50 hover:text-sky-700"
                        title="Refresh meta"
                      >
                        <RefreshCw className={`h-4 w-4 ${row.meta_loading ? "animate-spin" : ""}`} />
                      </button>
                      <button
                        type="button"
                ...
                <tr className="border-t border-slate-50 bg-slate-50/70">
                  ...
                  <td className="px-3 py-2" colSpan={5}>
                    <div className="space-y-0.5">
                      <div className="line-clamp-1 font-medium text-blue-700">{row.title || row.url}</div>
                      <div className="line-clamp-1 text-xs text-emerald-700">{row.domain || row.url}</div>
                      <div className="line-clamp-2 text-xs text-slate-600">
                        {row.description ||
                          (row.meta_loading
                            ? "Parsing description..."
                            : row.meta_error
                              ? "Meta parsing failed. You can retry."
                              : "No description")}
                      </div>
                    </div>
                  </td>
